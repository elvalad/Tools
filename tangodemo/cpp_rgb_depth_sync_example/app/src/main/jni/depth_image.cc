...
DepthImage::DepthImage()
    : texture_id_(0),
      ...
      gpu_texture_id_(0),
      depth_map_buffer_(0),
      center_point_depth_(0),
      depth_data_capturing_(false),
      center_region_depth_data_capturing_(false),
      center_depth_capture_frame_index_(0),
...
void DepthImage::UpdateAndUpsampleDepth(
  ...
  for (int i = 0; i < point_cloud_size; ++i) {
    ...
    //LOGE("Depth w: %d, Depth h : %d, Depth x : %d, Depth y : %d, Depth : %6f", depth_image_width, depth_image_height, pixel_x, pixel_y, depth_value);
   if ((std::abs(pixel_x - depth_image_width / 2) <= 3) && (std::abs(pixel_y - depth_image_height / 2) <= 3)) {
        center_point_depth_ = depth_value;
   }

    //if (center_point_depth_ != 0.000000) {
        //LOGE("center point depth in depth image : %6f", center_point_depth_);
    //}

    if (depth_data_capturing_) {
        //LOGE("Depth w: %d, Depth h : %d, Depth x : %d, Depth y : %d, Depth : %6f", depth_image_width, depth_image_height, pixel_x, pixel_y, depth_value);
        if (depth_data_output_file_.is_open()) {
            depth_data_output_file_ << pixel_x << " " << pixel_y << " " << depth_value <<"\n";
        }
    }

    if (center_region_depth_data_capturing_) {
        if ((std::abs(pixel_x - depth_image_width / 2) <= 100) && (std::abs(pixel_y - depth_image_height / 2) <= 100)) {
            if (center_region_data_output_file_.is_open()) {
                center_region_data_output_file_ << pixel_x << " " << pixel_y << " " << depth_value << "\n";
            }
        }
    }
  }
  ...
  if (depth_data_capturing_) {
    depth_data_capturing_ = false;
    depth_data_output_file_.close();
  }

  if (center_region_depth_data_capturing_ && center_depth_capture_frame_index_++ == 99) {
    center_region_depth_data_capturing_ = false;
    center_region_data_output_file_.close();
    center_depth_capture_frame_index_ = 0;
  }
}
...
float DepthImage::GetCenterPointDepth() {
    return center_point_depth_;
}

void DepthImage::SetDepthDataCapturing(bool on) {
    depth_data_capturing_ = on;
    if (depth_data_capturing_) {
        std::time_t time_now = std::time(nullptr);
        std::ostringstream oss;
        oss << "/sdcard/tmp/" << std::asctime(std::localtime(&time_now)) << ".data";
        depth_data_output_file_.open(oss.str());
    }
}

void DepthImage::SetCenterRegionDepthDataCapturing(bool on) {
    center_region_depth_data_capturing_ = on;
    if (center_region_depth_data_capturing_) {
        std::time_t time_now = std::time(nullptr);
        std::ostringstream oss;
        oss << "/sdcard/tmp/" << std::asctime(std::localtime(&time_now)) << "-center-region" << ".data";
        center_region_data_output_file_.open(oss.str());
    }
}
