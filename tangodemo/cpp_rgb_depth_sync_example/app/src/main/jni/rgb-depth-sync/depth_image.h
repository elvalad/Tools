...
class DepthImage {
 public:
  ...
  float GetCenterPointDepth();

  void SetDepthDataCapturing(bool on);

  void SetCenterRegionDepthDataCapturing(bool on);
 ...
 private:
  ...
  float center_point_depth_;

  bool depth_data_capturing_;

  std::ofstream depth_data_output_file_;

  bool center_region_depth_data_capturing_;

  int center_depth_capture_frame_index_;

  std::ofstream center_region_data_output_file_;
