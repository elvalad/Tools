...
#define IR_REGISTER_GAIN     0x35    		// todo: read address from configuration file
#define IR_REGISTER_EXPOSURE 0x09   		// todo: read address from configuration file
...
void switchColorToIR(int)
{
	if (g_bIsColorOn && !g_bIsIROn)
	{
		toggleStreamState(g_colorStream, g_colorFrame, g_bIsColorOn, openni::SENSOR_COLOR, "color");
		toggleStreamState(g_irStream, g_irFrame, g_bIsIROn, openni::SENSOR_IR, "IR");
	}
	else if (!g_bIsColorOn && g_bIsIROn)
	{
		toggleStreamState(g_irStream, g_irFrame, g_bIsIROn, openni::SENSOR_IR, "IR");
		toggleStreamState(g_colorStream, g_colorFrame, g_bIsColorOn, openni::SENSOR_COLOR, "color");
	}
...
void changeImageExposure(int delta)
{
	...
	{
		int value = 0;
		ReadDepthI2C(g_device, IR_REGISTER_EXPOSURE, value);

		int newValue = value + delta * 0x10;
		if (newValue < 1)
		{
			newValue = 1;
		}
		else if (newValue > 0x419)
		{
			newValue = 0x419;
		}

		// Patch to fix sensor timing issue
		WriteDepthI2C(g_device, 0x300A, 0x0400);
		WriteDepthI2C(g_device, 0x300C, 0x05D0);

		WriteDepthI2C(g_device, IR_REGISTER_EXPOSURE, newValue);
		ReadDepthI2C(g_device, IR_REGISTER_EXPOSURE, value);
		displayMessage("Changed exposure to: 0x%x", value);
	}
}
...
void changeImageGain(int delta)
{
	...
	{
		int value = 0;
		ReadDepthI2C(g_device, IR_REGISTER_GAIN, value);
		int gainValue = value + delta;
		if (gainValue < 0x08)
		{
			gainValue = 0x08;
		}
		else if (gainValue > 0x60)
		{
			gainValue = 0x60;
		}

		WriteDepthI2C(g_device, IR_REGISTER_GAIN, gainValue);

		ReadDepthI2C(g_device, IR_REGISTER_GAIN, value);
		displayMessage("Changed gain to: 0x%x", gainValue);
	}
}
