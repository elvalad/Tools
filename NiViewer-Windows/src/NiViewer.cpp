...
// Defines
// --------------------------------
#define APP_NAME_VERSION "NiViewer v1.2"
...
void createKeyboardMap()
{
	startKeyboardMap();
	{
		startKeyboardGroup(KEYBOARD_GROUP_PRESETS);
		{
			...
			registerKey('{', "Color to gray", toggleColorToGray, 1);
			registerKey('}', "Gray to color", toggleColorToGray, 0);
		}
		endKeyboardGroup();
 		startKeyboardGroup(KEYBOARD_GROUP_DEVICE);
 		{
			...
			registerKey('>', "Switch Color->IR/IR->Color", switchColorToIR, 0);
 		}
 		endKeyboardGroup();
...
// 	audioInit();
	...
	glutCreateWindow(APP_NAME_VERSION);
	//glutFullScreen();
