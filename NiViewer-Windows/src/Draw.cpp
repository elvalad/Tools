...
void YUV422ToRGB888(const XnUInt8* pYUVImage, XnUInt8* pRGBAImage, XnUInt32 nYUVSize, XnUInt32 nRGBSize)
{
	...
	while (pYUVImage <= pYUVLast && pRGBAImage <= pRGBLast)
	{
		...
		*/ 
		...
		if (g_bColorToGray)
		{
			pRGBAImage[0] = pRGBAImage[1] = pRGBAImage[2] = (piR[0] + piG[0] + piB[0]) / 3;
			pRGBAImage[3] = 255;
			pRGBAImage[4] = pRGBAImage[5] = pRGBAImage[6] = (piR[1] + piG[1] + piB[1]) / 3;
			pRGBAImage[7] = 255;
			pRGBAImage[8] = pRGBAImage[9] = pRGBAImage[10] = (piR[2] + piG[2] + piB[2]) / 3;
			pRGBAImage[11] = 255;
			pRGBAImage[12] = pRGBAImage[13] = pRGBAImage[14] = (piR[3] + piG[3] + piB[3]) / 3;
			pRGBAImage[15] = 255;
		}
 		...
 		else
		{
			...
			for (XnUInt16 nX = 0; nX < width; nX++, pTexture+=4)
			{
				...
				case openni::PIXEL_FORMAT_RGB888:
					...
					if (g_bColorToGray)
					{
						pTexture[0] = pTexture[1] = pTexture[2] = (pColor[0] + pColor[1] + pColor[2]) / 3;
					}
					pColor+=3; 
 					break;
...
void toggleColorToGray(int nColorToGray)
{
	if (nColorToGray == 1)
	{
		g_bColorToGray = true;
	}
	else if (nColorToGray == 0)
	{
		g_bColorToGray = false;
	}
}
