...
public:
	...
	Array(XnUInt32 nBaseSize = BASE_SIZE)
	{
		Init(nBaseSize);
	}

	/** Copy constructor. Initializes this array from another array of the same type. **/
	Array(const Array& other) : m_pData(NULL), m_nSize(0), m_nAllocatedSize(0)
	{
		*this = other;
	}
	...
	XnStatus CopyFrom(const Array& other)
	{
		if (this != &other)
		{
	...
	/** Zero the memory belonging to the array elements. **/
	void Zero()
	{
		memset(m_pData, 0, sizeof(T) * m_nAllocatedSize);
	}
